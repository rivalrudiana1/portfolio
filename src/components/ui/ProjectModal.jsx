...
const ProjectModal = ({ project, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
      {/* Backdrop */}
      <div
        className="absolute inset-0 bg-ulbi-blue/80 backdrop-blur-md cursor-pointer"
        onClick={onClose}
        aria-hidden="true"
      ></div>
      ...
      <div
        ...
        className="relative bg-white border border-ulbi-silver rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl"
      >
        {project.image && (
          <img
            src={project.image}
        ...
        <button
          ref={closeRef}
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-full bg-ulbi-grey/80 border border-ulbi-silver text-ulbi-blue/60 hover:text-ulbi-blue hover:bg-ulbi-silver transition-colors z-10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ulbi-orange"
        ...
        <div className="p-6 sm:p-8 md:p-10">
          <div className="mb-6">
            <h2 id="project-modal-title" className="text-3xl sm:text-4xl font-bold text-ulbi-blue mb-4 pr-10">{project.title}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10 mb-8 border-t border-ulbi-silver/60 pt-8">
            <div className="space-y-8">
              {project.challenge && (
                <div>
                  <h3 className="text-lg font-semibold text-ulbi-blue mb-3 flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-ulbi-orange" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              ...
              {project.solution && (
                <div>
                  <h3 className="text-lg font-semibold text-ulbi-blue mb-3 flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#001b59]" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex flex-col sm:flex-row gap-4 mt-auto">
                {(project.repoLinks ?? (project.githubUrl ? [{ label: 'Source Code', url: project.githubUrl }] : [])).map((repo) => (
                  <a
                    key={repo.url}
                    href={repo.url}
                    target="_blank"
